 ...
 * ## Compilation ##
 *
 *     gcc -Wall invalidate_path.c `pkg-config fuse3 --cflags --libs` -o
 * invalidate_path
 *
 * ## Source code ##
...
static int xmp_read(const char *path, char *buf, size_t size, off_t offset,
  ...
  if (strcmp(path, "/" TIME_FILE_NAME) == 0) {
    ...
    int to_copy = offset + size <= file_length ? size : file_length - offset;
    memcpy(buf, time_file_contents, to_copy);
    return to_copy;
  } else {
    assert(strcmp(path, "/" GROW_FILE_NAME) == 0);
    int to_copy =
        offset + size <= grow_file_size ? size : grow_file_size - offset;
